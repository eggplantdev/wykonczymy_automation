...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ALTER TYPE enum_cash_registers_type ADD VALUE 'VIRTUAL';
  `)
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  // Postgres does not support removing a value from an enum.
  // Recreate the enum without VIRTUAL and migrate existing rows.
  await db.execute(sql`
    UPDATE cash_registers SET type = 'AUXILIARY' WHERE type = 'VIRTUAL';
