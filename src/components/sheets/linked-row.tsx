...
// A sheet attached to an investment. The "Otwórz" CTA jumps to the
// per-investment sheet page (embedded sheet + sync controls). We assert
// `investment` is set — the listing page filters this in before rendering.
export function LinkedRow({ sheet }: PropsT) {
  const inv = sheet.investment!
  return (
    <li className="border-border bg-background flex flex-wrap items-center justify-between gap-3 rounded-md border p-3">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-muted-foreground truncate text-xs">{sheet.name}</p>
      </div>
