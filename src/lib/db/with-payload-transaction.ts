 ...
 *
 * `context` rides on `req.context`, which Payload forwards to afterChange hooks — `skipRevalidation`
 * (the default) for the kosztorys writes, `skipSheetSync` for the batched transfer create. Read the
...
export async function withPayloadTransaction<T>(
  ...
  context: TransactionContextT = { skipRevalidation: true },
): Promise<T> {
