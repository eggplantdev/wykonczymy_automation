...
// Input for the percent-rabat bulk-apply tool. A percent global rabat is not stored state — clicking
// „Zastosuj" writes `percent X` into every item's per-item rabat, so the only input is the percent.
//
...
export type ApplyPercentRabatT = z.infer<typeof applyPercentRabatSchema>
