...
- [x] fixed · impl-review/cohesion/scatter · `types/reference-data.ts` · `InvestmentStatusT` domain type lived inside the UI leaf `investment-status-badge.tsx`; hook + test imported a domain type from a component (inverted layering) — hoisted to `types/reference-data.ts`, now the single source (also collapsed the 3 inline `'active'|'completed'|'planowana'` unions in `InvestmentRefT`, `InvestmentRowT`, and the query cast into it).
- [x] fixed · scatter/cohesion · `components/investments/{investment-status-badge,status-filter}.tsx` · both are investment-specific but sat in generic `components/ui/`; feature-first home is `components/investments/` — moved both, updated all importers.
- [x] fixed · simplify (reuse/altitude, all 3 agents) · `app/(frontend)/inwestycje/[id]/page.tsx:80` · inline status→Polish-label map duplicated `STATUS_META` labels in the badge — extracted `STATUS_LABELS: Record<InvestmentStatusT,string>` (exported from the badge), consumed in both spots; badge split into `STATUS_LABELS` + `STATUS_CLASSNAMES`.
