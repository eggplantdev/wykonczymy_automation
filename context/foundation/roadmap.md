...
- **O-02: RWD — make the app usable on a phone** — EX-785 Done. Archived 2026-09-17 → `context/archive/2026-09-16-rwd-mobile/`. Phases 1–3 shipped (overflow root-caused, mobile drawer, shell switch `lg`→`sm`, dialog-as-sheet, collision-aware popover, the two named flows walked at 390px). Phase 4 („the rest") never scoped; the z-index retokenization deferred out → **EX-786**. Lesson: a `grid grid-cols-1` wrapper re-creates the `min-width: auto` overflow the flex shell already fixed with `min-w-0` — the guard has to be repeated per layout mode, it does not inherit.
