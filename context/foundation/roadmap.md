...
- **Planned follow-ons (docs-only on this branch, not yet built):** **EX-564** (`kosztorys-percent-rabat-bulk-apply`) — percent global rabat becomes a one-shot „Zastosuj" that writes per-item rabaty (Backlog); **EX-565** (`etap-tool-plane`) — per-etap z/bez-narzędzi plane + one view-independent podwykonawcy settlement (Todo). Both are Podsumowanie/kosztorys work tracked in Linear, plans written, no impl commits.
