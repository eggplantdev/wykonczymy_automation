...
| 11  | Stawka wykonawcy ma trzy źródła („auto" / mnożnik / kwota stała) i przełączenie między nimi zapisuje się w połowie — pozycja zostaje z dwoma źródłami naraz albo z żadnym, a rachunek podwykonawcy cicho zmienia kwotę | High   | Medium     | EX-865: EX-766 skasował parę kolumn właśnie dlatego, że siatka zapisuje jedno pole na wywołanie akcji, więc dwie kolumny nad jednym pojęciem utrwalały się jako dwa nieuporządkowane zapisy                     |
| 10  | The katalog-rozjazd counter lies or freezes the editor — it reports work already priced correctly, or the per-keystroke pass stalls typing on a large rozpiska | Medium | Medium     | change `katalog-problems`: the hint search costs 401 ms at 42 probes and 10.7 s at 400; one cena difference used to report as three                  |
...
| #11  | Każdy zapis źródła stawki zostawia pozycję z DOKŁADNIE jednym źródłem — normalizacja rozszerza zapis dotykający którejkolwiek kolumny pary na zapis obu — a wycena z mnożnika podąża za ceną j.m.                    | Że „akcja zwróciła sukces" znaczy, że para kolumn się zgadza; że `0` i `null` to ten sam stan (`0` to stawka ustawiona na zero, nie „auto") | Ścieżka zapisu komórki (jedno pole na wywołanie akcji) i jedno miejsce decydujące o pierwszeństwie mnożnik > kwota > globalny współczynnik | unit dla arytmetyki i normalizacji; dom dla przełącznika źródła i komórki tylko do odczytu     | Asercja na zwrotce akcji zamiast na zapisanym wierszu; test tylko „auto"→mnożnik, bez drogi powrotnej i bez wiersza z obiema kolumnami     |
