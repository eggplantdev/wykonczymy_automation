...
**In review** — all automated checks green (tsc, eslint, unit 14/14). The boxes below are the load-bearing device/platform gates that headless CI cannot cover; **2 of 4 ticked (2026-07-12)** — Safari native-decode and Chromium WASM fallback both confirmed locally. The two remaining are deploy-only (sharp derivative render + oversize guard on a Vercel preview).
...
- [ ] **Re-verify clean label after fix.** Upload a fresh receipt (scan path) → stored `filename` / opened-image label is `<opis>-<one-6hex>.<ext>` with **no** 30-char token and **no** double hex. Not yet re-checked post-fix.
