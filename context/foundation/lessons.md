...
## Listener `drop` na `window` musi wisieć w fazie przechwytywania — bo każda dropzone ucina bąbelkowanie, a `dragend` dla pliku z systemu nie istnieje

- **Context**: `drag-drop-guard` (2026-09-14) dokłada na `window` guard przeciw chybionemu dropowi (przeglądarka otwiera upuszczony plik jako dokument i gubi otwarty formularz) i przy okazji liczy zagnieżdżenia `dragenter`/`dragleave`, żeby wiedzieć, czy plik jest nad oknem.
- **Problem**: listenery zarejestrowane domyślnie, czyli w fazie bąbelkowej, **nigdy nie zobaczyły `drop` ani `dragleave` z wnętrza dropzone** — `FileInput.handleDrop`/`handleDragLeave` wołają `stopPropagation()`, a React i tak deleguje `drag*` na `document` (nie są w `nonDelegatedEvents`), więc ucięcie następuje poniżej `window`. `dragenter` docierał, `dragleave` nie: licznik rósł o +2/+3 na każde przejechanie nad polem i nie wracał do zera nawet po wyjeździe poza okno — słaby stan zostawał zapalony na zawsze. `dragend` nie jest ratunkiem: dla pliku ciągniętego z systemu operacyjnego **nie odpala się w ogóle** (źródło przeciągania jest poza dokumentem).
- **Rule**: (1) Globalny listener `drag*`/`drop`, który **liczy** cokolwiek, rejestruj z `{ capture: true }` — `window` jest wtedy pierwszy i żaden `stopPropagation` poniżej go nie uciszy; symetria `dragenter`/`dragleave` to warunek konieczny działania licznika. `preventDefault` w przechwytywaniu nie psuje własnego dropu dropzone (nie czyści `dataTransfer.files`). (2) Nie zastępuj licznika watchdogiem czasowym — `dragover` odpala się co ~350 ms, więc nieruchomy kursor dałby fałszywe wygaszenie. (3) Każdy element z „mocnym" stanem hoveru w trakcie przeciągania potrzebuje w `onDragLeave` guardu `currentTarget.contains(relatedTarget)`, bo `dragleave` odpala się także przy wjeździe kursora na **własne dziecko** (ikonę, `span`) — bez tego stan miga. (4) Globalny guard trzymaj idempotentnym (samo `preventDefault`, zero czytania pliku): przy N zamontowanych dropzone'ach na `window` wisi N kompletów listenerów i każdy zobaczy to samo zdarzenie.
- **Applies to**: każdy globalny listener zdarzeń, które komponenty poniżej mogą zatrzymać — `drag*`, `drop`, `paste`, `keydown`. Siostra lekcji o kolejności rejestracji i `stopImmediatePropagation` przy `paste` w react-datasheet-grid.

## „Niepotrzebne" w ustach właściciela bywa skrótem od „bo ma to zastąpić co innego" — a funkcja bez pokrycia browserowego znika, zanim ktokolwiek zauważy, że była używana

- **Context**: EX-672 skasowało 2026-08-12 wydruk transakcji razem z eksportem CSV, na zapis „obie funkcje niepotrzebne". Miesiąc później właściciel zgłosił, że wydruk był w użyciu, i trzeba go było napisać od nowa (`transfer-print-return`, 2026-09-14) — `git revert` nie wchodził, bo siedem modułów nie istniało już pod tymi nazwami.
- **Problem**: dwa zapisy w archiwum są ze sobą zgodne i mówią co innego. 2026-07-26 właściciel powiedział, że druk z przeglądarki jest wygaszany **na rzecz PDF-a**; 2026-08-12 ruling zanotowano jako „obie funkcje niepotrzebne", a plan-brief EX-672 dopisał „PDF replacement — None owed". Nikt nie zapytał, czy lipcowa obietnica została **odwołana**, czy tylko **odłożona** — a „niepotrzebne, **bo** przychodzi PDF" pasuje do obu zdań tak samo dobrze. Skasowano na podstawie drugiego czytania, nie sprawdzając pierwszego. Drugą połową jest to, że funkcja miała **zero testów browserowych**: nic nie zapiszczało przy usunięciu czterech stron-producentów, bo `TransferTableConfigT` miało te pola opcjonalne — `tsc` przechodził, przycisk po prostu przestawał istnieć.
- **Rule**: (1) Zanim skasujesz funkcję na „właściciel mówi, że niepotrzebna", **przeczytaj poprzednią wypowiedź właściciela o tej samej funkcji**. Jeśli wcześniej padło „zastąpimy to X", ruling jest niejednoznaczny dopóki nie potwierdzisz, że X nadal jest w planie — kasowanie zależne od nieistniejącego zamiennika to nie decyzja, to niedopowiedzenie. (2) **Pole opcjonalne w konfiguracji jest cichym wyłącznikiem funkcji.** `flag?: boolean` znaczy, że strona, która je zgubi, kompiluje się czysto i po prostu nie renderuje przycisku — żadnego sygnału z typecheckingu. Tam, gdzie opcjonalność jest wymuszona, weryfikacja każdego hosta jest ręczna i musi trafić do rejestru manualnych checków, nie do zdania w commicie. (3) Funkcja, której jedynym dowodem istnienia jest to, że ktoś ją klika, **nie ma dowodu istnienia**. (4) Symetrycznie w drugą stronę: świeże review czyta kod, nie archiwum, więc zgłosi jako bug każdą świadomą decyzję właściciela zapisaną tylko w `change.md` — wykluczenie anulowanych z tego wydruku zostało w ten sposób „naprawione" i cofnięte tego samego dnia. Decyzja właściciela, której kod nie tłumaczy komentarzem, jest findingiem czekającym na zgłoszenie.
- **Applies to**: /10x-plan (kasowanie funkcji), /10x-impl-review, code-review, slice-review-gate — i każde `flag?: boolean`, które włącza widoczny element UI

## Arkusz `react-datasheet-grid` jest bezwarstwowy, więc `@layer utilities` nigdy go nie pokona — barwienie komórek gridu idzie przez zmienną na wierszu + regułę w `globals.css`

- **Context**: Każde nadawanie koloru czemukolwiek wewnątrz `.kosztorys-grid` — tło paska sekcji, szyna `--section-rail`, ikona `⋯` w komórce akcji. Tailwind v4 wkłada utility do `@layer utilities`; `dsg-*` przychodzi z `node_modules` **bez** `@layer`, czyli spoza kaskady warstw.
- **Problem**: Reguła bez warstwy wygrywa z **każdą** regułą w nazwanej warstwie, niezależnie od specyficzności — `[&_svg]:text-(--section-rail,…)` na triggerze wyglądało poprawnie w kodzie i w DevToolsach jako przekreślone. Efekt: ikona zostawała na odziedziczonym `text-foreground`, więc właściciel zobaczył w każdej sekcji tę samą czarną kropkę i zgłosił to jako „kolor się nie zmienia". Diagnozę wydłużyło to, że pierwsze czytanie brzmiało „kolor działa, tylko jest wyblakły" — bo utility **istniało**, po prostu nigdy nie wygrywało.
- **Rule**: (1) Kolor komórki gridu ustawia się **zmienną na wierszu** (`rowClassName` → `--section-rail`) plus ręczną regułą pod `.kosztorys-grid` w `src/styles/globals.css`, nigdy utility na komponencie. (2) Kolor ikony musi być ustawiony **na ikonie** (`… svg { color: … }`) — wewnętrzny `span` triggera niesie `text-foreground`, które svg dziedziczy. (3) Utility przekreślone w DevToolsach przez regułę o niższej specyficzności = konflikt warstw, nie specyficzności; szukaj arkusza bez `@layer`, nie dokładaj `!important`. Uzasadnienie mechanizmu stoi w `src/lib/kosztorys/section-colors.ts`.
- **Applies to**: /10x-implement, /10x-impl-review, /simplify, tailwind-v4-audit — i każdy komponent renderowany wewnątrz `react-datasheet-grid`

## Bramka w menu musi czytać **ten sam predykat**, co handler, który firmuje — inaczej wyekstrahowana reguła pilnuje mostu, którego żadna ścieżka awarii nie przekracza

- **Context**: Komendy porządkowe w kosztorysie (▲/▼, „Wstaw sekcję powyżej/poniżej") wygaszane w dwóch menu, a odmawiane w trzech handlerach `use-kosztorys-editor.ts`. Ogólniej: każda para „UI decyduje, czy pokazać / handler decyduje, czy wykonać".
- **Problem**: Dwie klasy tej samej wady w jednym wycinku. (a) `sortActive` — predykat `orderCommandsEnabled(sort)` wyekstrahowano ze specem, ale czytało go **1 z 5 miejsc**; trzy handlery dalej miały gołe `if (sort) return`, więc spec przypinał most, którym nic nie chodziło. (b) `sectionNeighbor`/`neighborSectionId` zwracają `undefined` na krawędzi bloku, handler cicho wychodzi, a menu nadal pokazywało czynne ▲ — właściciel kliknął pierwszą pracę w sekcji i zapytał „co to robi? Wygląda na to, że nic". Obie to ta sama pułapka: **wygląda na czynne, nie robi nic**.
- **Rule**: (1) Kiedy wyekstrahujesz predykat bramki, **policz `grep`-em jego wywołania i porównaj z liczbą miejsc, które podejmują tę decyzję** — ekstrakcja czytana w jednym miejscu jest gorsza niż jej brak, bo spec daje fałszywe poczucie pokrycia. (2) Warunek, przy którym handler **cicho wychodzi**, jest z definicji warunkiem, przy którym menu musi być wygaszone — przejrzyj każde wczesne `return` handlera jako kandydata na `disabled`. (3) Spec takiej bramki asercjonuje **równoważność z handlerem** (`canMoveItem(...) === (sectionNeighbor(...) !== undefined)`), nie osobną tabelkę prawdy — inaczej oba mogą się rozjechać zielono.
- **Applies to**: /10x-plan, /10x-implement, /10x-impl-review, /simplify, code-review, slice-review-gate

## Status inwestycji to etykieta, nie bramka — a dodanie czwartej wartości do unii wywala typecheck w 3 miejscach na ~10, reszta psuje się po cichu

- **Context**: `szablony-crud` (2026-09-14) dokłada status `szablon` dla ukrytej inwestycji-warsztatu, w której edytuje się szablony kosztorysu. Research zmapował pełny promień rażenia czwartej wartości `InvestmentStatusT`.
...
## `Where` z `access` mówi wyłącznie o wierszu **zapisanym** — więc bramka na `update` bez drugiego ramienia jest zapadnią, a nie zamkiem

- **Context**: EX-770 domyka zamek zakończonej inwestycji na piątej kolekcji (`kosztoryses`) — jedynej, którą `/admin` potrafił przepiąć mimo zamka. 2026-09-15. Fakty niżej sprawdzone w źródle Payloada 3.73.0 **i** odpalone przeciwko `db-test`, nie wywnioskowane.
- **Problem**: reguła `access` zwracająca `Where` wygląda na komplet, a odpowiada tylko na pytanie „czy wolno ruszyć **ten** wiersz", nigdy „czy wolno go przestawić **tam**". `investment.status != completed` przepuszcza więc zapis, który wsadza arkusz **do** zakończonej inwestycji: dopasowanie leci po stanie SPRZED zapisu. Po zapisie ten sam `Where` zamyka wiersz od środka — `/admin` go już nie ruszy, warstwa akcji odbija przez `lockedSheetError`, a odkręcenie to SQL na bazie. Zapadnia, nie zamek. Do tego trzy pułapki mechaniki: (a) **`not_equals` po nullowalnej relacji NIE odsiewa wierszy bez relacji** — Payload kompiluje to do `(kolumna IS NULL OR kolumna <> wartość)`, a trawersowanie relacji jest LEFT JOINem, więc niepowiązany arkusz przechodzi; **`not_in` tak NIE działa** i podmiana zamroziłaby każdy arkusz bez inwestycji; (b) `access` rzuca **przed** hookiem na `create`/`update`, więc hook postawiony tam na komunikat jest martwym kodem — jedyna operacja, na której hook zdąży się odezwać, to `delete`; (c) odmowa z `access`-`Where` dociera jako **403**, a w praktyce użytkownik nawet nie klika Zapisz, bo panel renderuje dokument read-only.
- **Rule**: (1) **Bramka na `update` ma dwa ramiona albo jest dziurawa** — `Where` na wierszu zapisanym plus odczyt przychodzącego `data` (Payload podaje `data` do `executeAccess` dla `update`, i to jest jedyny powód, dla którego złożenie jest wykonalne). Ten sam test stosuj do każdej reguły, która bramkuje **relację**: jedno ramię pilnuje wyjścia, drugie wejścia. (2) Fabryka reguł dostępu, która bierze i „właściciela relacji", i „ścieżkę `Where`", **fail-open bez niczego, co by to złapało** — sparuj `item` z `investment.status` i bramka pilnuje nie tego przeskoku, po cichu i zielono. Wyprowadzaj ścieżkę z właściciela mapą `as const satisfies`, nie przyjmuj obu. (3) Składając regułę bazową, **`await base(args)`** — typ `Access` dopuszcza `Promise<AccessResult>`, więc asynchroniczna reguła ról bez `await` daje `allowed !== true`, fabryka zwraca Promise, Payload rozwiązuje go do `true` i **zamek znika**. TypeScript to przyjmuje bez słowa. (4) Spec takiej bramki asercjonuje **stan trwały po odmowie** (`investment_id` wiersza), nie zwróconą wartość — a jeśli tabela ma unikalny indeks na bramkowanej kolumnie (tu: 1:1 `investment_id WHERE NOT NULL`), **każdy przypadek potrzebuje własnej fikstury docelowej**: indeks odbija zapis przed konsultacją z bramką, więc współdzielone cele przechodzą na zielono także po cofnięciu bramki. Ten spec złapał się na to dwa razy.
- **Applies to**: /10x-plan, /10x-implement, /10x-impl-review, code-review, slice-review-gate — każda reguła `access` zwracająca `Where` po relacji, i każda fabryka takich reguł

## Nowa kolekcja Payloada kosztuje w migracji dziewięć rzeczy, z których żadnej nie widać w konfiguracji kolekcji

- **Context**: `katalog-sprzetu` (2026-09-01) dokłada trzy kolekcje naraz (`equipment`, `equipment-events`, `warehouses`) jedną ręcznie pisaną migracją. Lista niżej to inwentarz zrobiony przed jej napisaniem, przeciwko `20260818_1_add_fleet.ts` i `20260901_0_add_work_catalogue_items.ts` — wzorcom, które przeszły prod.
- **Problem**: `CREATE TABLE` z pól kolekcji wypisuje się sam i wygląda na komplet. Nie jest. Payload trzyma połowę schematu **obok** tabeli, w strukturach, o których konfiguracja kolekcji nie mówi ani słowa, i każda pominięta wysypuje się dopiero na żywym żądaniu: (1) **`payload_locked_documents_rels` potrzebuje kolumny + indeksu na KAŻDĄ nową kolekcję** — panel SELECT-uje ją przy każdym żądaniu i bez niej rzuca; pominięto to przy `expense_categories` i poszła migracja naprawcza (`20260310_fix_locked_docs_expense_categories.ts`). (2) **`hasMany` (relacja albo upload) to nie skalarny FK, tylko ręczna tabela `<table>_rels`** — pięć statementów i cztery indeksy (`20260818_1:73-90`). (3) **`created_at` / `updated_at` plus ich dwa indeksy są obowiązkowe** na każdej tabeli Payloada. (4) **Pole `date` to `timestamp(3) with time zone` przypięte do północy UTC, nigdy `date`** — to jedyny kształt, jaki adapter modeluje; porównania dni robi JS. (5) **`unique: true` w Payloadzie jest jednokolumnowe** — nadaje się na numer seryjny, nie na klucz złożony. (6) **Rejestracja w `src/migrations/index.ts` jest ręczna** (import + wpis). (7) **Indeks złożony pod dominującą ścieżkę odczytu** trzeba dołożyć świadomie — Flota ma `(vehicle_id, type, performed_at DESC)`, log sprzętu `(equipment_id, occurred_at DESC)`; bez niego „gdzie to jest" to seq scan po całym logu. (8) **Seed słownika należy do migracji**, `INSERT … ON CONFLICT (name) DO NOTHING` (`20260309_add_expense_categories.ts:19-23`). (9) **`down()` odwraca w dokładnie odwrotnej kolejności, łącznie z `DROP TYPE`** — enum przeżywa `DROP TABLE`.
- **Rule**: (1) Zanim napiszesz migrację nowej kolekcji, przejdź tę dziewiątkę jako listę kontrolną — kompilator i `payload generate:types` nie pilnują ani jednej pozycji, a `migrate:create` nie zastąpi listy, bo emituje fantomowy drift (AGENTS.md). Nagłówek pliku: `// Hand-written (migrate:create's snapshot baseline is stale — see AGENTS.md).` (2) **FK do słownika wybiera się po tym, czy referencja jest OPISEM czy CELEM.** `expense_categories` dostało `ON DELETE SET NULL`, bo wydatek bez kategorii dalej jest wydatkiem; magazyn jest **celem przekazania**, więc wyzerowanie zostawia historyczny wpis bez żadnego celu — złamany niezmiennik i fałszywy alarm „nie wiadomo gdzie". Taki FK bierze `ON DELETE RESTRICT` **plus sondę `makePreventDelete`**, żeby odmowa doszła jako zdanie, nie jako błąd Postgresa. Nie kopiuj kierunku z poprzedniej migracji — rozstrzygnij pytanie na nowo. (3) Nazewnictwo pliku i kierunek wdrożenia mają własne reguły — patrz „Name migrations so filename sort matches dependency order" wyżej i „Migrations" w `AGENTS.md`.
- **Applies to**: /10x-plan, /10x-implement, /10x-impl-review, code-review — każda migracja zakładająca kolekcję Payloada

## Payload połyka nierozwiązywalną ścieżkę sortowania i po cichu wraca do `-createdAt` — więc nieważny `?sort=` nie jest błędem, tylko INNĄ wiarygodną kolejnością

- **Context**: EX-777 (2026-09-15) przenosi sortowanie tabeli transakcji z klienta do bazy. Fakty niżej sprawdzone w źródle Payloada (`buildOrderBy`), nie wywnioskowane z zachowania.
- **Problem**: `payload.find({ sort })` z kluczem, którego nie da się rozwiązać do kolumny — literówka, kolumna relacyjna przy `depth: 0`, ręcznie wklejony śmieć — **nie rzuca i nie ostrzega**. `buildOrderBy` cicho odrzuca taką ścieżkę i zapytanie leci z `-createdAt`. Skutek jest gorszy niż błąd: użytkownik dostaje listę posortowaną, wiarygodną i **nie tę**, o którą prosił. Kiedy dwa kanały sięgają po ten sam zbiór własnymi kluczami (ekran przez host strony, wydruk przez akcję serwerową wołaną z klienta), jeden może trafić w kolumnę, a drugi w fallback — i rozjazd jest niewidoczny po obu stronach, bo każda z nich wygląda na posortowaną poprawnie. Drugi koszt: nieważna wartość dokłada własny wpis do klucza `unstable_cache`, więc śmieciowy URL zatruwa cache jedną pozycją na wartość.
- **Rule**: (1) Klucz sortowania z URL-a przepuszczaj przez **białą listę, i przez JEDNĄ** — nie jedną na kanał. Kanał wołany z klienta (akcja serwerowa pod przyciskiem) rewaliduje ten klucz u siebie, nawet jeśli strona, która wyrenderowała przycisk, już go zwalidowała; inaczej whitelista pilnuje tylko tej drogi, którą i tak nikt nie atakuje. (2) Walidator zwraca `undefined` dla wartości spoza listy, nie wartość domyślną — wtedy ten sam helper obsłuży hosta strony (`?? DEFAULT`), akcję (spada do własnej domyślnej) i stan tabeli w kliencie (pusto → brak strzałki w nagłówku). Zwrócenie domyślnej zamyka trzeciego konsumenta: pusty URL zaczyna malować sortowanie, którego nikt nie wybrał. (3) **Sortowalność kolumny wyprowadzaj z białej listy, nie deklaruj przy kolumnie.** Dziesięć ręcznych `enableSorting: false` to dziesięć miejsc na rozjazd i spec, który go pilnuje; `isServerSortable(column.id)` to zero. Po wyprowadzeniu asercja „każda sortowalna kolumna jest na liście" robi się tautologią — zostaw tylko odwrotną („wpis listy to kolumna, którą tabela renderuje"), bo ta dalej łapie realny dryf. (4) Kolumna pokazująca nazwę z innej tabeli przy `depth: 0` **nie jest sortowalna** — wiersz niesie samo id, nazwa dokleja się po pobraniu strony. Zawęża się ją filtrem.
- **Applies to**: /10x-plan, /10x-implement, code-review, slice-review-gate — każdy parametr URL-a, który dociera do `payload.find` jako `sort`, i każda tabela sterowana z URL-a

## Hook `beforeValidate` dostaje **złączony dokument**, nie łatkę — więc „które pola przyszły" jest zawsze prawdą, a test karmiony ręczną łatką tego nie złapie

- **Context**: Każdy hook Payloada, który próbuje odróżnić „ten zapis dotyka tylko pola X" od zwykłej edycji — tu wyjątek `invoiceOnly` w `src/hooks/transfers/invoice-only-patch.ts`, jedyna dozwolona zmiana na transakcji zakończonej inwestycji (EX-748). Rozpoznane przy ręcznej weryfikacji, nie przez testy.
- **Problem**: Warunek czytał **klucze** `data` („przyszła tylko faktura"). Payload podaje hookowi `beforeValidate` cały zapisany dokument z nałożoną łatką, więc kluczy jest zawsze komplet — warunek nie był prawdziwy **nigdy** poza testem jednostkowym, który karmił go ręcznie sklejoną łatką. Skutek: podpięcie skanu do transakcji zakończonej inwestycji kończyło się odmową („Inwestycja jest zakończona i tylko do odczytu…"), czyli jedyny zamierzony wyjątek był martwy. Test jednostkowy świecił się na zielono przez cały ten czas, bo wysyłał kształt, który nie przychodzi z produkcji.
- **Rule**: (1) Nie wnioskuj o **intencji zapisu** z kształtu `data` — porównaj **wartości** względem zapisanego wiersza i uznaj za „tylko X", gdy każde inne pole jest równe temu, co już stoi w bazie. (2) Spec jednostkowy hooka musi dostać kształt, który naprawdę przychodzi z Payloada (pełny dokument), nie minimalną łatkę wygodną do napisania — inaczej pinuje wyobrażenie o frameworku, nie framework. (3) Regułę na granicy z frameworkiem stawia się specem **DB-owym** (`*.db.test.ts`), bo tylko on przepuszcza prawdziwy `payload.update` przez prawdziwy hook.
- **Applies to**: /10x-implement, /10x-impl-review, code-review, /10x-tdd — każdy hook kolekcji rozstrzygający „co się zmieniło"

## Goły `throw new Error` w hooku Payloada nie dociera do człowieka — `routeError` wyciera komunikat, którego nie potrafi uznać za publiczny

- **Context**: `src/hooks/transfers/validate.ts` i każdy inny hook kolekcji, który odrzuca zapis regułą biznesową. Wychodzi na jaw dopiero przez `/admin` albo gołe wywołanie REST — aplikacja ma własną walidację Zodem po stronie klienta, więc ta ścieżka nie jest widoczna w normalnym użyciu.
- **Problem**: Hook zbierał naruszenia do tablicy i robił `throw new Error(errors.join(' '))`. Payload przepuszcza to przez `routeError`, który podmienia komunikat wszystkiego, czego nie potrafi uznać za publiczny — API oddaje `500 {"errors":[{"message":"Something went wrong."}]}`. Zapis jest **poprawnie odrzucony** (baza nietknięta), ale OWNER/MANAGER w panelu widzi generyczną awarię zamiast powodu. Maskowanie nie było zamierzone: część tych komunikatów jest po polsku i pisana wprost do człowieka („Zaksięgowanej transakcji nie można zmienić"), a komentarz dwa bloki wyżej w tym samym pliku nazywał goły rzut defektem — odpowiedź na „czy to celowe" stała w pliku, w którym postawiono pytanie.
- **Rule**: (1) Reguła biznesowa odrzucająca zapis rzuca `APIError(komunikat, 400)`, nigdy `Error` — `Error` jest dla awarii, `APIError` dla odmowy. (2) Zanim zapytasz właściciela, czy zachowanie jest zamierzone, przeczytaj komentarze **w tym samym pliku**; „wymaga człowieka" postawione nad odpowiedzią, która już tam stoi, kosztuje tydzień. (3) Strażnik jest tani i należy do granicy: spec pinujący **typ** rzutu i status (`APIError` + 400 + treść), nie samą odmowę — odmowa działała przez cały czas.
- **Applies to**: /10x-implement, /10x-impl-review, code-review — każdy hook kolekcji odmawiający zapisu

## Wersjonowany klucz `unstable_cache` chroni dokładnie tyle, ile ktoś pamięta — trwałym strażnikiem jest konsument, który nie jest właścicielem kształtu

- **Context**: `src/lib/queries/notification-recipients.ts` (`fetchRecipientLists`, klucz `['notification-recipients']`) po tym, jak EX-758 dołożyło `equipmentDigest` do `RECIPIENT_LISTS`. Rozszerza lekcję „Widening a type that flows through an `unstable_cache` payload needs a KEY bump" o to, czego bump nie załatwia.
- **Problem**: Wpis w cache'u powstał **przed** dodaniem klucza i nic go nie przelicza przy deployu — wygasa wyłącznie przez `updateTag(NOTIFICATION_RECIPIENTS_TAG)` z akcji zapisu, której po tym deployu nikt nie zawołał. `recipients.equipmentDigest` rozpakowało się więc do `undefined`, a `RecipientListCard` czytające `emails.length` wywaliło **całą stronę `/sprzet` każdemu odwiedzającemu** w granicę błędu — przy poprawnym renderze serwerowym (RSC oddawał komplet danych; `"emails":"$undefined"` widać dopiero w serializowanych propsach klienta). Spec na `readRecipientLists()` („zwraca każdy klucz z `RECIPIENT_LISTS`") **nie złapałby tego** — ta funkcja mapuje po rejestrze z `?? []`, więc wypełnia każdy klucz i dziś, i w zepsutym buildzie. Zły kształt przyszedł z CACHE'U, nie z czytnika.
- **Rule**: (1) Bump klucza jest mitygacją zależną od pamięci człowieka — trwały strażnik siedzi na **konsumencie**: komponent, który rozpakowuje pole, którego kształtu nie jest właścicielem, musi znieść jego brak (`emails?.length ?? 0`), bo to on pada. (2) Nie pisz specu na czytnik, gdy defekt jest w cache'u — sprawdź najpierw, czy proponowana asercja przechodzi na zepsutej wersji; jeśli tak, jest tautologią. (3) **Nieświeży wpis odtyka się bez deployu**: zawołaj dowolną akcję, która robi `updateTag` na tym samym tagu (tu: zapisz z UI niezmienioną inną listę odbiorców) — jeden tag unieważnia wpis obsługujący wszystkie listy. To pierwsza rzecz do zrobienia przy takim wysypie na produkcji, przed jakimkolwiek redeployem. (4) Obraz odwrotny: po zapisie, który zmienia treść **poza aplikacją** (restore bazy, przełączenie branchu), bezterminowy wpis trzyma stare liczby w nieskończoność — figury finansowe z takiego środowiska cross-check'uj SQL-em, zanim uznasz rozjazd za defekt.
- **Applies to**: /10x-implement, /10x-impl-review, code-review, verify-manual-checks — każde poszerzenie kształtu przechodzącego przez `unstable_cache`

## `formId` szkicu formularza nazywa **typ** formularza, a nie jego instancję — okno przypięte do encji jest osobną instancją i potrzebuje klucza

- **Context**: `create-form-store.ts` / `use-managed-form.ts` i każde okno `add-*`, które bywa otwierane z karty konkretnej encji. Ujawnione ręczną weryfikacją floty: „Nowy przegląd".
- **Problem**: Kontrakt store'a mówi to wprost — „One slot per form type, but ‚Dodaj pojazd' and ‚Edytuj pojazd 7' are two instances of it" — a `use-managed-form.ts` przywraca szkic przy `storedFormId === formId`. „Nowy przegląd" otwarty ze strony pojazdu jest przypięty do tego pojazdu (`lockedVehicleId`), czyli jest **inną instancją**, a wszystkie szły pod jednym `formId="add-inspection"`. Porzucony szkic przeciekał więc na inny pojazd i przeżywał pełną nawigację. Objaw był mylący — mieszanka: pole „Pojazd" resetowało się (bo `mergeStored` je nadpisuje), reszta nie — co czyta się jak kaprys, a nie jak złamany kontrakt.
- **Rule**: (1) Okno, które niesie **zablokowaną encję**, dostaje `formId` sparametryzowany jej id (`inspectionDraftId(vehicleId)`), a wariant ogólnofirmowy — gdzie encja jest jeszcze polem — zostaje przy wspólnym slocie. (2) Przeciek szkicu między encjami to defekt, nie „wygoda przy powtarzanym wprowadzaniu danych" — nie pytaj o to właściciela, przeczytaj kontrakt store'a. (3) Kiedy jedno pole resetuje się, a reszta nie, szukaj **dwóch** mechanizmów (przywracanie szkicu + nadpisanie po montażu), nie jednego kapryśnego.
- **Applies to**: /10x-implement, /10x-impl-review, code-review — każde nowe okno formularza przypięte do encji

## Jedno pojęcie w dwóch kolumnach edytora zapisuje się DWOMA niezależnymi łatkami — więc ma stan, którego żaden pojedynczy zapis nie potrafi wyprodukować

- **Context**: EX-766 (2026-09-02), para `*_override_type` (`'amount' | NULL`) + `*_override_value` (`numeric NOT NULL DEFAULT 0`) na pozycji kosztorysu, zwinięta do jednego `overrideValue: number | null`. Pierwszy przebieg researchu uznał stan nielegalny za **nieosiągalny** i dopiero drugi to obalił — ta lekcja jest o tym, dlaczego pierwszy się mylił.
- **Problem**: `subcontractorPolicy.clear` zwraca `{type: null, value: 0}` jako **jeden obiekt**, więc na poziomie helpera zmiana trybu wygląda atomowo. Do bazy jednym obiektem nie dociera nigdy: `itemPatchSchema` jest `.partial()`, `grid-change-plan.ts` spłaszcza diff wiersza do **jednego wpisu na zmienione pole**, a edytor odpala `updateItemFieldAction(id, { [field]: value })` osobno dla każdego wpisu wewnątrz `Promise.all`. Zmiana trybu utrwala się więc jako **dwa niezależne, równoległe, nieuporządkowane zapisy jednokluczowe**. Pomiędzy nimi wiersz naprawdę jest `{type: null, value: 500}` — a jeśli drugi zapis przegra (sieć, zamknięta karta, 401), ten stan jest **trwały**. `/admin` dochodzi do niego wprost, dwoma zwykłymi polami. Nic tego nie złapało, bo każdy odczyt rozgałęział się na **typie** — czyli dokładnie na tej połowie pary, którą zwinięcie usuwało.
- **Rule**: (1) Dwie kolumny niosące jedno pojęcie są atomowe **najwęższą ścieżką zapisu**, a w edytorze arkuszowym tą ścieżką jest pojedyncze pole. Helper zwracający parę jako jeden obiekt **nie jest dowodem atomowości** — prześledź wartość aż do akcji serwerowej, zanim uznasz stan za nieosiągalny. (2) Migracja zwijająca taką parę **nie przenosi wartości wprost**: `CASE WHEN <dyskryminator> THEN value ELSE NULL END`. Gołe `value` wskrzesza osieroconą kwotę jako prawdziwą. (3) Argumentem za zwinięciem jest to, że stan nielegalny staje się **niewyrażalny** — nie to, że unia z jednym członem jest śmieszna. Ta druga wersja nie przetrwa pierwszego pytania „to po co to ruszać". (4) **Zwinięcie pary to jedna z dwóch odpowiedzi, i zwykle gorsza** — dopisane po EX-865 (2026-09-23), które przywróciło wycięty tryb jako **własną kolumnę** `*_override_coeff` obok `*_override_value`. Zmienną rozstrzygającą jest **ścieżka zapisu, nie liczba kolumn**: para jest atomowa wtedy i tylko wtedy, gdy akcja serwerowa zapisuje **całe pojęcie** jednym UPDATE-em (`normalizeOverridePatch` dostawia drugą kolumnę do łatki, która wymieniła pierwszą). Kasowanie trybu, żeby uzyskać atomowość, płaci za nią funkcjonalnością i wraca jako zgłoszenie — a wracając, wraca na tę samą ścieżkę zapisu, która nadal jest jednopolowa. Napraw ścieżkę zapisu **zanim** zaproponujesz cięcie kolumny.
- **Applies to**: /10x-plan, /10x-research, /10x-implement, code-review — każde pojęcie rozłożone na dwie kolumny zapisywane z gridu albo z `/admin`

## Zmiana kolumny wpiętej w hash golden mastera nie wywala testu — ona WYPISUJE inwestycję z porównania

- **Context**: EX-766 (2026-09-02). `financial-golden-master-db.test.ts` miesza obie kolumny override w per-inwestycyjny hash `overrides`; zwinięcie pary zmieniało kształt tych kolumn.
- **Problem**: zmieniony hash robi `inputsUnchanged()` fałszywym, co przenosi inwestycję do `dataMoved` i **pomija porównanie jej kwot**. To nie jest czerwony test — to cichy ubytek pokrycia: podłoga stoi na `total/2`, więc 11 inwestycji na 115 wypada bez jednego ostrzeżenia. Gorzej, baseline był już nieuczciwy **przed** zmianą: zatwierdzony fixture powstał, gdy dump nie miał ani jednego wiersza kosztorysu, więc wszystkie jedenaście inwestycji z kosztorysem — czyli dokładnie te, których kwoty ta zmiana mogła ruszyć — przechodziło refaktor **nieporównywane**. Ok. 46 tys. zł robocizny mogło zniknąć na zielonym teście. Druga pułapka jest w samym wyrażeniu: „oczywista" normalizacja (`case when type = 'amount' then value::text else '' end`) **przesuwa każdy hash** — wiersz auto emitował `':0::0'`, a po niej `':'`, czyli wywołuje dokładnie tę regenerację, której miała zapobiec.
- **Rule**: (1) Zanim ruszysz kolumnę karmiącą hash golden mastera, **najpierw zregeneruj fixture na świeżo zaimportowanej bazie** — inaczej diffujesz przeciwko baseline'owi, który już niczego nie pilnuje. (2) Nowe wyrażenie hasha pisz tak, żeby **odtwarzało stare bajty jako literały** i nie wymieniało kasowanej kolumny z nazwy — wtedy jest poprawne po obu stronach deployu i żaden odcisk się nie rusza. Sprawdź to tabelką stan-wiersza → stare bajty → nowe bajty, nie oceną „wygląda równoważnie"; trafienie `numeric` w `::text` jest bajtowe. (3) Czytaj „N pominiętych" jak czerwony, nie jak zielony. Komunikat „przeseeduj i zregeneruj" jest właśnie tym gumowym stemplem, przed którym ostrzega lekcja o golden masterze wyżej.
- **Applies to**: /10x-plan, /10x-implement, /10x-impl-review, code-review — każda zmiana kolumny wchodzącej w `inputHashes` golden mastera

## Składowany klucz dopasowania to zobowiązanie do backfillu — appka porównuje ŚWIEŻY klucz ze SKŁADOWANYM, więc rozjechany wiersz znika cicho

- **Context**: clean-texts-catalogue-names (2026-09-15). 253 reguły całonazwowe weszły do `foldDescription`, czyli do `catalogueKey` i `itemKey` naraz, przy 843 wierszach katalogu z `UNIQUE` na `work_catalogue_items.match_key`.
- **Problem**: kusi myśl „`match_key` to kolumna wyliczana przy zapisie, więc poszerzenie folda nie unieważnia starych wierszy". Nieprawda i to groźnie: `buildCatalogueComparison` buduje `byKey` ze **składowanych** `entry.matchKey`, a szuka w nim **świeżo policzonym** `catalogueKey(description, unit)`. Wiersz, któremu fold przesunął klucz, przestaje być osiągalny — praca raportuje się jako „spoza katalogu", choć siedzi w katalogu. `partitionAlreadyInKosztorys` ma to samo: porównuje składowany `item.matchKey` ze świeżym zbiorem, więc praca już wstawiona pokaże się jako niewstawiona i wjedzie drugi raz. Ścieżki zapisu jadą na `ON CONFLICT DO NOTHING`, czyli duplikat **cicho nie powstaje** zamiast wywalić błąd. Zmiana funkcji klucza re-keyuje obliczenie, nie wiersze — i to jest dokładnie ta luka.
- **Rule**: (1) Zmieniasz `fold` / `foldDescription` / `catalogueKey` / `itemKey` → **domyślnie jesteś winien backfill** składowanej kolumny. Zwolnić z niego może tylko pomiar, nigdy argument „przecież klucz jest składowany". (2) Pomiarem jest spec driftu: per wiersz `catalogueKey(opis, j.m.) !== match_key` na prawdziwych danych — sama unikalność świeżo liczonych kluczy tego nie widzi. Spec **nie zastępuje migracji**, tylko mówi, czy ją musisz napisać; czerwony = backfill, nie wzruszenie ramionami. (3) Każdy taki spec dostaje podłogę na rozmiarze zbioru (`> 500`), inaczej zielone znaczy „nie było danych". (4) Tutaj backfill nie był należny **z powodu kierunku tabeli**: poprawki mapują stare pisownie → nazwy poprawione, a katalog trzymał już te poprawione (wpisy tożsamościowe wypadają z pochodnej), więc 253 reguły przesuwają stronę **rozpisek** na klucze katalogu, a nie klucze katalogu. Zmierzone: 0 rozjechanych wierszy z 843 na kopii produkcji. Tabela poszerzona w drugą stronę — o nazwę, która dziś **jest** w katalogu — złamie to natychmiast. (5) Gotowa reguła rozstrzygania kolizji przy zapisie do katalogu (wygrywa wiersz bez „[stary arkusz]", przy remisie niższe id) leży w historii: `4de2666e^:src/scripts/fix-work-catalogue-texts.ts`, `resolveCollisions`.
- **Applies to**: /10x-plan, /10x-implement, code-review — każda zmiana funkcji klucza, której wynik jest gdziekolwiek **składowany** w kolumnie

## `min-w-0` nie dziedziczy się przez tryby układu — `grid grid-cols-1` odtwarza dokładnie to przepełnienie, które flexowa powłoka już naprawiła

- **Context**: rwd-mobile / O-02 (2026-09-16), poziome przepełnienie strony na telefonie. Powłoka miała już poprawkę `min-w-0` na flexowej kolumnie (`app/(frontend)/layout.tsx`), a kontenery, które „powinny" ciec (tabela transakcji, siatka kosztorysu), miały własne `overflow-x-auto`. Mimo to strona jeździła w bok i przyczyny nie dało się zgadnąć z kodu.
- **Problem**: `ui/page-wrapper.tsx` jest `grid grid-cols-1`, a element siatki — tak samo jak element flexa — ma `min-width: auto`, więc rośnie do szerokości swojej **treści**, zamiast pozwolić własnemu `overflow-x-auto` zadziałać. To jest ta sama awaria, którą powłoka naprawiła piętro wyżej: poprawka jest lokalna dla trybu układu, nie propaguje się w dół drzewa. Kuszący obejściem jest `overflow-x-hidden` na `body` — i on **maskuje przyczynę**: strona przestaje jeździć, a kontener dalej jest za szeroki, tylko nikt już tego nie widzi.
- **Rule**: (1) Każdy wrapper, który wprowadza nowy kontekst formatowania (`flex`, `grid`), jest osobnym miejscem, gdzie `min-width: auto` może zjeść `overflow-*` dziecka — poprawka należy się **każdemu z osobna** (`[&>*]:min-w-0` dla siatki jednokolumnowej). (2) Przyczynę przepełnienia **mierzy się w przeglądarce**, nie zgaduje z kodu: kandydaci z `overflow-x-auto` wyglądają na winnych i nimi nie są. (3) Backstop `overflow-x-hidden` dopiero **po** znalezieniu przyczyny — przed nią zamienia defekt na niewidzialny defekt.
- **Applies to**: /10x-plan, /10x-implement, code-review, responsive-audit — każda praca nad przepełnieniem poziomym i każdy nowy wrapper układu na trasie strony

## Link w kontenerze bez pudełka układu nigdy się nie prefetchuje — więc na telefonie `loading.tsx` nie działa przy PIERWSZEJ nawigacji, tylko przy następnej

- **Context**: rwd-mobile / O-02 (2026-09-16). Właściciel na telefonie: tapnięcie w pozycję menu zamykało szufladę natychmiast, po czym przez kilka sekund nie działo się nic. Po odświeżeniu ta sama nawigacja była szybka.
