...
## `match_key` jest liczony przy zapisie, więc poszerzenie folda nie unieważnia katalogu — ryzyko dotyczy wyłącznie NOWYCH wierszy

- **Context**: clean-texts-catalogue-names (2026-09-15). 253 reguły całonazwowe weszły do `foldDescription`, czyli do `catalogueKey` i `itemKey` naraz, przy 843 wierszach katalogu pod `UNIQUE` na `work_catalogue_items.match_key`.
- **Problem**: odruch mówi „zmiana funkcji klucza = migracja albo przebieg hurtowy po całej tabeli". Nieprawda: `match_key` to kolumna, nie wyrażenie — niesie klucz **wyliczony w momencie zapisu**, więc stare wiersze nikogo nie obchodzą, dopóki ich nikt nie nadpisze. Pęka co innego: dwa wiersze, które dotąd miały różne klucze, pod nowym foldem zbiegają się w jeden, a ścieżki zapisu (seed katalogu, wstawka pracy z katalogu, `item-to-catalogue`) jadą na `ON CONFLICT DO NOTHING` — czyli drugi wiersz **cicho nie powstaje** zamiast wywalić błąd. Stąd spec kolizji przeciw `db-test` z podłogą na liczbie wierszy: bez podłogi przechodzi jałowo na pustym katalogu i nie pilnuje niczego.
- **Rule**: (1) Zanim zaplanujesz migrację pod zmianę funkcji klucza, sprawdź, czy klucz jest **składowany** czy **liczony w zapytaniu** — składowany nie potrzebuje migracji, potrzebuje testu kolizji na dzisiejszych danych. (2) Spec kolizji asertuj **per wiersz przeciw składowanemu kluczowi** (`key !== match_key`), nie tylko unikalność świeżo liczonego zbioru — realny tryb awarii to rozjazd świeżego klucza ze składowanym. (3) Każdy taki spec dostaje podłogę na rozmiarze zbioru, inaczej zielone znaczy „nie było danych". (4) Gotowa reguła rozstrzygania kolizji przy zapisie do katalogu (wygrywa wiersz bez „[stary arkusz]", przy remisie niższe id) leży w historii: `4de2666e^:src/scripts/fix-work-catalogue-texts.ts`, `resolveCollisions`.
- **Applies to**: /10x-plan, /10x-implement, code-review — każda zmiana `fold` / `foldDescription` / `catalogueKey` / `itemKey`
