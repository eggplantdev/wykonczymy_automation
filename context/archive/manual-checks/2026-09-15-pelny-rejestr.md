...
- [x] **Global top-nav overflows horizontally below ~571px, breaking the app's own 768px mobile line** — `document.body.scrollWidth` (571) exceeds `window.innerWidth` (390) on `/inwestycje/135/kosztorys_v2` at 390px width; traced (`browser_evaluate`, walking the widest child at each level) to `src/components/nav/top-nav.tsx`'s `<header>` — a plain `flex … justify-between gap-3` row with no `flex-wrap`/`min-w-0` handling for its logo + `investmentCrumb` breadcrumb + right-side action buttons. `TopNav` is shared by every investment sub-page, so this isn't scoped to EX-618 or the kosztorys editor — found incidentally while driving Phase 3's narrow-screen boxes. Poziomy scroll całej strony plus 768px jako jedyna linia mobile→desktop tej aplikacji czynią to regresją powtórzoną na każdej trasie, nie problemem kosztorysu. (Pierwotny zapis powoływał się tu na „regule stylistycznej AGENTS.md” — takiego zdania w AGENTS.md nie ma i nigdy nie było; cytat sprostowany 2026-09-15.) **ODPOWIEDZIANE — patrz „ROZWIĄZANE" niżej. (Pierwotnie 2026-09-04:)** Design decision explicitly named in the entry — how to reshape `src/components/nav/top-nav.tsx` at narrow widths (truncate breadcrumb / wrap buttons / hide logo). Shared by every route, out of scope to fix unilaterally; needs a design call before a fix lands. **ROZWIĄZANE (2026-09-15, staging):** diagnoza „potrzebna decyzja projektowa dla `TopNav`" była błędna — nagłówek nie był sprawcą. Rozpychał stronę sąsiad: kolumna treści w `src/app/(frontend)/layout.tsx` nie miała `min-w-0`, więc siatka kosztorysu (~2310 px kolumn) nadawała minimalną szerokość całej kolumnie, a pasek górny jechał razem z nią. Poprawka (`min-w-0`) jest w `origin/staging` (`src/app/(frontend)/layout.tsx:64`). Pomiar na żywo na stagingu, `/inwestycje/135/kosztorys_v2`: `document.body.scrollWidth === window.innerWidth` przy 390, 360, 571 i 767 px (`overflow: false` na każdej z czterech szerokości; przed poprawką 571 vs 390). Treść nie jest przycięta, tylko przewijalna wewnątrz swojego kontenera: przy 390 px `.dsg-container` ma `clientWidth 390 / scrollWidth 2310`, a poziome kółko przesunęło jej `scrollLeft` 424 → 1924 przy niezmienionym `document.body.scrollWidth` 390. Żadna zmiana w `top-nav.tsx` nie była potrzebna, więc decyzja projektowa odpada. **Test disposition:** no automated test — spec `e2e/shell-narrow-viewport.spec.ts` był napisany i został **wycofany 2026-09-15** decyzją właściciela: godzina pakietu e2e plus seed współdzielonej bazy to za dużo za jedną klasę CSS w aplikacji, której nikt nie obsługuje przy 390 px, a objaw (strona jedzie w bok) widać natychmiast. Strażnikiem zostaje komentarz przy `min-w-0` w `src/app/(frontend)/layout.tsx` — tłumaczy, dlaczego ta klasa tam stoi, więc kto ją kasuje, kasuje najpierw uzasadnienie.
...
- [ ] Break the Meta token in `.env`, hit the route with the correct secret → **500** _and_ a „🚨 Cron odzyskiwania zgłoszeń nie zadziałał" mail lands in the „Alerty techniczne" list. This is the failure the whole change exists to prevent, and the only leg no unit test can prove end-to-end (real Graph rejection → real SMTP send). **Wymaga człowieka (2026-09-04):** Same constraints as the two boxes above — no staging `CRON_SECRET`, and breaking a live Meta token + observing a real "Alerty techniczne" mail delivery are both off-limits on a non-production environment per this task's absolute prohibitions and `AGENTS.md`'s mail-gate rule.
      ...
      **Przebieg 2026-09-15 (lokalnie, DB testowa na 5435, token Meta celowo zepsuty) — box zwężony,
      nadal otwarty.** Aplikacja podniesiona na 3010 z `META_PAGE_ACCESS_TOKEN=DELIBERATELY-BROKEN-FOR-QA`
      przekazanym wyłącznie w środowisku procesu — `.env` nietknięty, produkcja nietknięta. Zweryfikowane
      po kolei:
      • kontrola autoryzacji: zły sekret → **HTTP 401**, bez wejścia w handler.
      • poprawny `CRON_SECRET` → **HTTP 500** z `{"error":"Reconcile failed"}`.
      • odrzucenie przez Grapha jest prawdziwe, nie zasymulowane:
      `Graph request failed for 104897439055542/leadgen_forms?fields=id,name,leads_count&limit=100
    (status 401)`. `listLeadForms()` rzuca przed pętlą, więc leci zewnętrzny `catch`, a nie ścieżka
      `failedForms` — dokładnie ta gałąź, której ten box dotyczy.
      • `alertSweepFailure` wystartował: odbiorcy `opsAlerts` rozwiązani z bazy (gdyby lista była pusta,
      `requireRecipients` rzuciłby „Brak odbiorców"; zamiast tego doszło do samej wysyłki), po czym
      `payload.sendEmail` padł na `getaddrinfo ENOTFOUND disabled.invalid` — czyli na bramce pocztowej
      z `AGENTS.md`, a nie na błędzie kodu. Zalogowane jako
      `[cron/leads-reconcile] Failure alert failed`.
      Czyli cały łańcuch **odrzucenie Grapha → 500 → złożenie maila alertowego do właściwej listy** jest
      potwierdzony na żywym odrzuceniu Mety. **Jedyne, co zostaje, to samo doręczenie SMTP**, które poza
      produkcją jest nieobserwowalne z definicji — i to jest powód, dla którego box zostaje niezahaczony.
