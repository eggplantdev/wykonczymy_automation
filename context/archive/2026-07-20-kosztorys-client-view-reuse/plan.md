...
2. **Owner editor IS changed — ratified.** The plan promised "owner editor unchanged," but the verbose column labels („Pomiar (razem etapy)", „Razem Netto", „…względem przedmiaru/pomiaru"), the taller wrapping header row, and the rewritten header tips are ungated on `clientView` and so apply to the owner grid too. Owner ratified this as intended in both views (the labels disambiguate „względem przedmiaru" vs „względem pomiaru"). The "owner unchanged" line in §19/§21 is superseded.
   - **Post-merge refactor (`baee9b68`) widened the owner-facing set — also ratified:** the money-axis toggle label „Oba" → „Pokaż wszystko" (the toggle renders in both the client slim header and the owner toolbar), and a new revoke-confirm guard — „Wyłącz link" now opens a `ConfirmDialog` spelling out the revoke is irreversible before deleting the client's live `kosztorys-shares` row (previously a one-click destructive delete). Both are deliberate and outside the original plan scope; recorded here so this addendum stays the exhaustive list of owner-visible changes.
